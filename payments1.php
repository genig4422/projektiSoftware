...
$current_date = date('Y-m-d'); // Set to current date (2025-06-09)
...
try {
    ...
    // Fetch payment records for current date
    $query = "SELECT p.payment_id, p.reservation_id, p.amount, p.payment_date, p.status, 
              ...
              WHERE c.business_id = ? AND DATE(p.payment_date) = ?";
    $params = [$business_id, $current_date];
    $types = "is";
    ...
    // Group payments by date (though only one date will be shown)
    $grouped_payments = [];
...
<body>
    <div class="main-content" id="mainContent">
        <div class="container my-5">
            <h2 class="text-center mb-4">Payment Management (Today: <?= htmlspecialchars($current_date) ?>)</h2>
            <?php if ($success): ?>
            ...
            <div class="form-container mb-4">
                <form method="GET" action="payments1.php" class="row g-3">
                    <div class="col-md-6">
            ...
            <?php if (empty($grouped_payments)): ?>
                <div class="alert alert-warning">No payments found for today (<?= htmlspecialchars($current_date) ?>).</div>
            <?php else: ?>
